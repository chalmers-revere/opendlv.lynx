...
m_sampleTime = 0.1f;
m_states = Eigen::MatrixXd::Zero(5,1);
m_delta = 0.0f;
}
...
void BicycleModel::nextContainer(odcore::data::Container &a_container)
{
  ...
  if (a_container.getDataType() == opendlv::logic::action::AimPoint::ID()) {
    ...
    m_delta = Kp*aimPoint.getAzimuthAngle();
  }
...
void BicycleModel::setUp()
{
  ...
  //%%%%%%%%%%%%VEHICLE MODEL PARAMETERS%%%%%%%%%%%%%%%%%%%
	float const vM = 188; //kg     //kv.getValue<float>("sim-lynx-bicyclemodel.vehicle-parameter.m");
	float const vIz = 105; //kgm²  //kv.getValue<float>("sim-lynx-bicyclemodel.vehicle-parameter.Iz");
	float const vG = 9.82; //m/s²  //kv.getValue<float>("sim-lynx-bicyclemodel.vehicle-parameter.g");
	float const vL = 1.53; //m     //kv.getValue<float>("sim-lynx-bicyclemodel.vehicle-parameter.l");
	float const vLf = 0.765; //m (l*0.5) //kv.getValue<float>("sim-lynx-bicyclemodel.vehicle-parameter.lf");
	float const vLr = 0.765; //m (l*0.5) //kv.getValue<float>("sim-lynx-bicyclemodel.vehicle-parameter.lr");
	float const vMu = 0.9;         //kv.getValue<float>("sim-lynx-bicyclemodel.vehicle-parameter.mu");
	m_vehicleModelParameters << vM,vIz,vG,vL,vLf,vLr,vMu;

  //%%%%%%%%%%%%System Params%%%%%%%%%%%%%%%%%%%%%%
  //m_sampleTime << float const 0.1; //kv.getValue<float>("sim-lynx-bicyclemodel.T");
}
...
Eigen::MatrixXd BicycleModel::vehicleModel(Eigen::MatrixXd &x)
{
	...
	Eigen::Eigen::MatrixXd xdot = Eigen::MatrixXd::Zero(6,1);
